...
- Next, we look at the hour. Figure 6. and Figure 7. show us the time for most of the accidents that occurred is 5 pm. The data still consistent. 
...
- From 2003 to 2015, there is a significant difference between the number of accidents on the weekend vs. the weekdays (Figure 8.). It looks like accidents tend to appear on the weekend rather than on weekdays.
...
p <- tmp %>%
  ...
  geom_line(aes(frame = Year))+
  ggtitle("Figure 8. Accidents relate in hour and weekend overall years")
ggplotly(p)
```
...
ggplot(tmp, aes(x = "", fill=`Injury Type`)) + 
  geom_bar()+
  ggtitle("Figure 9. Injury types  overall from 2003 to 2015") +
  theme_minimal()+
  coord_polar("y")
```
...
ggplot(Month, aes(x = "", fill=`Injury Type`)) + 
  geom_bar()+
  ggtitle("Figure 10. Injury types  in from 2003") +
  theme_minimal()+
  coord_polar("y")
```
...
tmp %>%
  ...
  ggplot(aes(x = Month, y = count)) + 
  geom_col() + 
  ggtitle("Figure 11. Collision types from 2003 to 2015") +
...
tmp %>%
  ...
  ggplot(aes(x = Month, y = count)) + 
  geom_col() + 
  ggtitle("Figure 12. Collision types in 2003") +
  facet_wrap(~`Collision Type`)
...
ggplot(tmp, aes(x = fct_rev(fct_infreq(`Primary Factor`)))) + 
  geom_bar()+
  ggtitle("Figure 13. Primary factors for accidents from 2003 to 2015") +
...
ggplot(Month, aes(x = fct_rev(fct_infreq(`Primary Factor`)))) + 
  geom_bar()+
  ggtitle("Figure 14. Primary factors for accidents in 2003") +
  theme_minimal()+ coord_flip()
```


```{r}
Fin<-filter(Month, Month == "10", wday=="Friday", hour=="17")
```
