...
ggplot(Month, aes(x = Month)) + 
  ...
  ggtitle("Fig. 1. Accidents in 2003") +
  theme_minimal()
```

- October has the most accidents reported (477) in 2003. 
- We then look at the overall trends of which month are likely to result in more crashes.
...
p <- tmp %>%
  ...
  geom_line(aes(frame = Year))+
  ggtitle("Fig. 2. Overall accidents from 2003 to 2015") 
ggplotly(p)
...
ggplot(tmp, aes(x = `Weekend?`)) + 
  ...
  ggtitle("Weekday or weeekend in 2003") +
  theme_minimal()
```
...
p <- tmp %>%
  ...
  ggplot(aes(x = hour, y = count, color = `Weekend?`)) + 
  geom_line(aes(frame = Year))+
  ggtitle("Weekday or weeekend overall from 2003 to 2015")
ggplotly(p)
```


- For the year of 2003, the primary factor that for most of the accidents is "Failure to yield right of way"
```{r, fig.height=20}
library(forcats)
